...
public class AuthUserQueryReport<T extends AbstractEntity> extends PdfDocument {
	...
	private PdfPTable createFirstPageTitleTable() throws DocumentException {
		...
		final GenericTableBuilder titleTableBuilder = new GenericTableBuilder(TITLE_CELL_FONT, firstPageTitle);
	...
	private String getFirstPageTitle() {
		...
		if(authUserEntityQuery != null) {
			...
			firstPageTitle = I18nUtils.getInternationalizedMessage(queryLanguage, "queryReport.title", null);
		} else {
			firstPageTitle = I18nUtils.getInternationalizedMessage("en", "queryReport.title", null);
		}
	...
	private PdfPTable createUserTable() throws DocumentException {
		...
		if(authUserEntityQuery != null) {
			...
			userTitle = I18nUtils.getInternationalizedMessage("en", "queryReport.user", null);
	...
	private PdfPTable createQueryUrlTable() throws DocumentException {
		...
		final GenericTableBuilder searchUrlTableBuilder = new GenericTableBuilder(columnName, columnValue, URL_TABLE_COLUMN_WIDTH);
	...
	private PdfPTable createQueryResultTable() throws DocumentException {
		...
		if(LogicalUtils.isNotNullNorEmpty(contentArray)) {
			...
			final GenericTableBuilder searchResultTableBuilder = new GenericTableBuilder(columnNames, null, RESULT_TABLE_COLUMN_WIDTHS, genericTableContentDTO);
			searchResultTable = searchResultTableBuilder.build();
		} else {
			final String emptyResultText = getEmptyResultText();
			final GenericTableBuilder titleTableBuilder = new GenericTableBuilder(EMPTY_RESULT_CELL_FONT, emptyResultText);
	...
	private String[] getColumnNames(final String[] columnMessageNames) {
		...
		if(columnMessageNames != null) {
			...
			for(String columnMessageName : columnMessageNames) {
				final String columnName = I18nUtils.getInternationalizedMessage(queryLanguageCode, columnMessageName, null);
	...
	private String getEmptyResultText() {
		...
		if(authUserEntityQuery != null) {
			final Language queryLanguage = authUserEntityQuery.getLanguage();
			emptyResultText = I18nUtils.getInternationalizedMessage(queryLanguage, "queryReport.emptyResultText", null);
		} else {
			emptyResultText = I18nUtils.getInternationalizedMessage("en", "queryReport.emptyResultText", null);
		}

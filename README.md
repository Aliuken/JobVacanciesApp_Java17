...
The code was created under the **[Apache 2.0 license](https://github.com/Aliuken/JobVacanciesApp_Java17/blob/main/LICENSE)** using the **SOLID principles** and the **SOA architecture**.
...
* **MVC**: Through @Controller classes (pkg: [com.aliuken.jobvacanciesapp.controller](https://github.com/Aliuken/JobVacanciesApp_Java17/tree/main/src/main/java/com/aliuken/jobvacanciesapp/controller)) from Spring MVC and using the utility class MvcUtils.
...
* **Singleton**: It is used when we want a single instance of a class and it is not possible to use enums, @Autowired or utility classes (with only static methods). This pattern is used in the packages:
    ...
    * [com.aliuken.jobvacanciesapp.util.javase](https://github.com/Aliuken/JobVacanciesApp_Java17/tree/main/src/main/java/com/aliuken/jobvacanciesapp/util/javase): To get the EnumUtils instance statically to execute its instance methods (that make use of generics).
    * [com.aliuken.jobvacanciesapp.util.javase.stream](https://github.com/Aliuken/JobVacanciesApp_Java17/tree/main/src/main/java/com/aliuken/jobvacanciesapp/util/javase/stream): To get StreamUtils instances (of type SequentialStreamUtils or ParallelStreamUtils) statically to execute its instance methods.
...
* **OpenJDK 17**: As the Java SE implementation.
...
* **Internationalization** (**i18n**): Configured in [LocaleConfig](https://github.com/Aliuken/JobVacanciesApp_Java17/blob/main/src/main/java/com/aliuken/jobvacanciesapp/config/LocaleConfig.java) (as in <https://www.baeldung.com/spring-boot-internationalization>) and built for:
...
* **Spring MessageSource class**: To deal with internationalization (i18n). It is configured in [LocaleConfig](https://github.com/Aliuken/JobVacanciesApp_Java17/blob/main/src/main/java/com/aliuken/jobvacanciesapp/config/LocaleConfig.java).
...
* **Bootstrap 5.3.3** and **Material Design for Bootstrap 7.3.0**: For the look-and-feel.
* **Font Awesome Free For The Web 6.5.2**: For the application icons.
...
* **TinyMCE Community 7.1.1**: For the rich text editor.
...
>
> Other Thymeleaf fragments (in [src/main/resources/templates/fragments](https://github.com/Aliuken/JobVacanciesApp_Java17/tree/main/src/main/resources/templates/fragments)) are:
...
* **iText Core itextpdf 5.5.13.3**: As the Java library to export queries to PDFs.
...
* **Docker Compose for the app** (in [docker-compose-app/docker-compose.yaml](https://github.com/Aliuken/JobVacanciesApp_Java17/blob/main/docker-compose-app/docker-compose.yaml)). It uses:
    * In **app-db-service**: The latest MySQL Docker image ("mysql:latest").
    * In **app-service**: The file [docker-compose-app/Dockerfile](https://github.com/Aliuken/JobVacanciesApp_Java17/blob/main/docker-compose-app/Dockerfile), which uses "amazoncorretto:17-alpine-jdk" (a Docker image with JDK 17 for Alpine Linux).
* **Docker Compose for the Elastic Stack** (in [docker-compose-elastic-stack/docker-compose.yaml](https://github.com/Aliuken/JobVacanciesApp_Java17/blob/main/docker-compose-elastic-stack/docker-compose.yaml)). It uses:
    * The **Elastic Stack 8.13.4** Docker images (url: <https://www.docker.elastic.co>): For analyzing the app log files, which pass through the stack in the next order: "Filebeat &rArr; Logstash &rArr; Elasticsearch &rArr; Kibana".
...
In the file [docker-compose-app/docker-compose.yaml](https://github.com/Aliuken/JobVacanciesApp_Java17/blob/main/docker-compose-app/docker-compose.yaml):
...
In the file [docker-compose-elastic-stack/docker-compose.yaml](https://github.com/Aliuken/JobVacanciesApp_Java17/blob/main/docker-compose-elastic-stack/docker-compose.yaml):
...
> * [EnumUtils](https://github.com/Aliuken/JobVacanciesApp_Java17/blob/main/src/main/java/com/aliuken/jobvacanciesapp/util/javase/EnumUtils.java): To manage ConfigurableEnum elements.
>
...
* **jobvacanciesapp.defaultAnonymousAccessPermissionValue**: Indicates whether anonymous users can enter the application or not. Initial value: "false". Possible values (defined in the enum **AnonymousAccessPermission**):
...
* **jobvacanciesapp.defaultPdfDocumentPageFormatCode**: Indicates the page format for the query PDF documents. Initial value: "A4V". Possible values (defined in the enum **PdfDocumentPageFormat**):
...
* **jobvacanciesapp.defaultUserInterfaceFrameworkCode**: Indicates the UI framework of the application. Initial value: "M". Possible values (defined in the enum **UserInterfaceFramework**):
...
* **jobvacanciesapp.defaultAnonymousAccessPermissionValueOverwritten**: Overwrites the anonymous-access configuration type (defined in the property "jobvacanciesapp.defaultAnonymousAccessPermissionValue"). Possible values (defined in the enum **AnonymousAccessPermission**):
...
* **jobvacanciesapp.defaultPdfDocumentPageFormatCodeOverwritten**: Overwrites the page format for the query PDF documents (defined in the property "jobvacanciesapp.defaultPdfDocumentPageFormatCode"). Possible values (defined in the enum **PdfDocumentPageFormat**):
...
* **jobvacanciesapp.defaultInitialTablePageSizeValueOverwritten**: Overwrites the default initialTablePageSize (defined in the property "jobvacanciesapp.defaultInitialTablePageSizeValue"). Possible values (defined in the enum **TablePageSize**):
    ...
    * **5**, **10**, **25**, **50**, **100**, **250** and **500**: Meaning the quantity specified by the name.
* **jobvacanciesapp.defaultUserInterfaceFrameworkCodeOverwritten**: Overwrites the UI framework of the application (defined in the property "jobvacanciesapp.defaultUserInterfaceFrameworkCode"). Possible values (defined in the enum **UserInterfaceFramework**):

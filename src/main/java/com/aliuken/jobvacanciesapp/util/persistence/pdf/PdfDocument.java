...
public abstract class PdfDocument extends Document implements AutoCloseable {
	...
	private static final Font REGULAR_FONT = new Font(FontFamily.HELVETICA, 7);
	private static final Font BOLD_FONT = new Font(FontFamily.HELVETICA, 7, Font.BOLD);
	...
	protected static void addCellWithPhrase(final List<PdfPCell> pdfPCellList, final String fieldName, final String fieldValue) {
		...
		if(fieldName != null && !fieldName.isEmpty()) {
			phrase = new Phrase();
			phrase.add(new Phrase(StringUtils.getStringJoined(fieldName, ": "), PdfDocument.BOLD_FONT));
			phrase.add(new Phrase(fieldValue, PdfDocument.REGULAR_FONT));
		} else {
			phrase = new Phrase(" ", PdfDocument.REGULAR_FONT);
		}
